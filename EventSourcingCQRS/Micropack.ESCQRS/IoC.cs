...
public static class IoC
{
    ...
    {
        services.AddScoped<IBusAccessor, BusAccessor>();
    ...
    public static void AddCommandHandlersFromAssemblyContaining<TCommandHandler, TCommand, TCommandResponse>(this IServiceCollection services, ServiceLifetime serviceLifetime = ServiceLifetime.Scoped)
       ...
       where TCommandHandler : ICommandHandler<TCommand, TCommandResponse>
    {
        ...
        var commandHandlers = assembly.GetTypes()
                ...
                .Select(type => new
                {
                    ...
                    TCommand = type.GenericArguments[0],
                    TCommandResponse = type.GenericArguments[1],
                })
                .ToList();

        commandHandlers.ForEach(type =>
        {
            var handlerType = typeof(IRequestHandler<,>).MakeGenericType(type.TCommand, type.TCommandResponse);
            services.Add(new ServiceDescriptor(handlerType, type.THandler, serviceLifetime));
        });
    }
